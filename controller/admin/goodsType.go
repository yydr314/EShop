...
func (con GoodsTypeController) DoAdd(ctx *gin.Context) {
	...
	if err != nil {
		con.Error(ctx, "增加角色失敗 請重試", "/admin/goodsType/add")
	} else {
...
func (con GoodsTypeController) DoEdit(ctx *gin.Context) {
	...
	if title == "" {
		con.Error(ctx, "角色標題不能為空", "/admin/goodsType/edit")
		return
	}
	...
	if err != nil {
		...
		con.Success(ctx, "修改數據成功", "/admin/goodsType/role")
	}
}
...
func (con GoodsTypeController) Auth(ctx *gin.Context) {
	...
	ctx.HTML(http.StatusOK, "admin/goodsType/auth.html", gin.H{
		"roleId":     roleId,
		"accessList": accessList,
	})
}

func (con GoodsTypeController) DoAuth(ctx *gin.Context) {
